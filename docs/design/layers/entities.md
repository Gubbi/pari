...
`entity` is the sole layer that has no orchestration tier: every boundary here emits `PrimitiveError` directly ([layer-model.md](layer-model.md#layer-structure)). Generated behavior from `#[derive(Entity)]` and `entity_registry!` is consumed by `workspace`, `store`, and `substrate` — each owns its piece of the output (see [Generation Contract](#generation-contract)).
...
`AnyEntityRef` is the type-erased sum of all `EntityRef<T, P>` shapes, used wherever refs of mixed kinds must flow through one value (e.g., cross-entity ref collection). Conversion happens through an instance method on `EntityRef<T, P>`: `entity_ref.to_any_ref()` returns the matching `AnyEntityRef` variant. The implementation is generated by the derive — call sites simply work with the typed ref they already hold.
...
| `TrackedField::loaded(value)` | no | Field-level constructor used by the store's JSON-to-tracked conversion when wrapping a substrate-loaded or insert-time JSON payload into a tracked entity. |
| `TrackedField::mutated(value)` | yes | Setter path. Produces the replacement `Arc` for COW swap. |

### Copy-on-write mutation

A setter does not mutate the existing `TrackedField`. It builds a new `Arc<TrackedField::mutated(v)>` and swaps the `Arc` on the editor's owned `TrackedX` instance. The store's canonical `Arc` for that field is unchanged until `commit` merges the dirty fields back. Concurrent readers holding earlier `Arc` clones see the old value. The dirty flag on the new field marks which fields the store must persist.

### Construction is store-mediated

`Tracked<Name>` itself carries only state and `pub(crate)` helpers; there is no public constructor. The path from a plain entity (or substrate-loaded JSON) into a `Tracked<Name>` runs through the store's JSON-to-tracked pipeline, which uses `TrackedField::loaded` per field internally. `TrackedEntity::from_json_value` — the type-erased counterpart — is `pub(crate)` and reachable only through that pipeline.
...
|---|---|---|
| `Tracked<Name>` struct (fields wrapped in `Arc<TrackedField<T>>`) | `entity` | The tracked state companion used throughout the runtime. State only — no async surface. |
| `impl Entity for <Name>` (with `KIND`, `Parent`, `Tracked`, `validation_schema()`, `extract`, `take`, `into_tracked_entity`) | `entity` | Entity identity glue and the per-kind dispatch hooks the registry calls. |
| `impl TrackedFor for Tracked<Name>` | `entity` | Roundtrip: `Tracked<Name>::Entity = Name`. |
| `EntityRef<Name, Parent>::to_any_ref(&self)` | `entity` | Instance-method conversion to `AnyEntityRef`. |
| Custom `Serialize` / `Deserialize` for the tracked companion | `entity` | Serde uses the plain shape; load path funnels through `TrackedField::loaded`. |
| `XViewer<'ws, Name>` with per-field async accessors and `validate` / `validate_with` | `workspace` | Read handle returned by `Workspace::resolve` / `Workspace::import`; sibling reads and validation flow through it. |
| `XEditor<'ws, Name>` with `Deref<Target = XViewer<'ws, Name>>`, per-field setters, and `commit(self)` / `undo_checkout(self)` | `workspace` | Mutation handle returned by `Workspace::checkout::<Name>`. Owns the checked-out tracked instance; not `Clone`; consumes itself on lifecycle terminators. |
| Validation-schema access | `workspace` | Connects the entity to its rule set inside the workspace's validation sub-area. |
...
| `TrackedEntity` validation dispatch | `workspace` |
