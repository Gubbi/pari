...
The system SHALL define a `Storage` trait and an `EntityStore` struct in `src/storage/mod.rs`. `EntityStore` is a neutral container of all validated entity collections; it carries no "repo" concept. The trait exposes `persist(&self, store: &EntityStore) -> Result<(), Vec<StorageError>>`. What varies across implementations is the persistence target, not what is stored. Future capabilities (e.g., `load`) will be added to this trait in subsequent proposals.

#### Scenario: RepoStorage implements Storage
- **WHEN** `RepoStorage` is used as a `Storage` implementor
- **THEN** it satisfies the trait contract and can be used wherever `Storage` is required

---

### Requirement: RepoStorage accepts a caller-provided root path
The system SHALL provide `RepoStorage::new(root: impl Into<PathBuf>) -> Self` in `src/storage/repo/storage.rs`. The root directory is determined entirely by the caller. No default path (e.g., `.pari/`) is hardcoded inside the storage layer.

#### Scenario: Arbitrary root path accepted
- **WHEN** `RepoStorage::new` is called with any valid path
...
### Requirement: StorageError identifies the failing path
Each `StorageError` SHALL carry a `path` (the filesystem path that failed, as a string) and a `message` (human-readable description of the failure).

#### Scenario: Write failure produces StorageError with path
- **WHEN** writing `roles/eng-lead.md` fails due to a permission error
- **THEN** the returned `StorageError` has `path: "roles/eng-lead.md"` and a descriptive message
