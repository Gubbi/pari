...
The system SHALL define a `Substrate` trait in `src/substrate/mod.rs`. The trait exposes `persist(&self, changeset: &ChangeSet) -> Result<(), Vec<SubstrateError>>`. The substrate receives a pre-built `ChangeSet` and persists only the changes described in it. The substrate does not interact with `EntityStore` directly — change detection is the caller's responsibility via `EntityStore::drain_changes()`.
...
#### Scenario: persist accepts a ChangeSet
- **WHEN** `substrate.persist(&changeset)` is called with a ChangeSet containing one modified role
